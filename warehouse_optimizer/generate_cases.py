...
# Case 17: Famous problem of packing 17 unit squares in a square
def gen_case17():
    p = make_case_dir(17)
    # The smallest square that can contain 17 unit squares has a side length of ~4.67553.
    # We scale by 1000, so warehouse is 4676 x 4676 and bays are 1000 x 1000.
    s = 4676
    warehouse = [[0, 0], [s, 0], [s, s], [0, s]]
    write_csv(f"{p}/warehouse.csv", warehouse)
    
    write_csv(f"{p}/obstacles.csv", []) # No obstacles
    
    ceiling = [[0, 5000], [s, 5000]]
    write_csv(f"{p}/ceiling.csv", ceiling)
    
    bays = []
    # id, width, depth, height, gap, loads, price
    bays.append([0, 1000, 1000, 1000, 0, 1, 1000])
    write_csv(f"{p}/types_of_bays.csv", bays)
